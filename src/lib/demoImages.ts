...
// サーバーサイドProxy経由で画像生成（APIキーはCloudflare Pages Functionが保持）
export async function createDemoImageAI(index: number): Promise<string | null> {
  if (index < 0 || index >= DEMO_PROMPTS.length) return null;
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[DemoImage] API returned ${res.status}`);
      return null;
    }
    const data = await res.json();
    if (data.image) {
      console.log(`[DemoImage] Generated via server proxy`);
      return data.image;
    }
    return null;
  } catch (e) {
    console.warn("[DemoImage] API error:", e);
    return null;
  }
}
